    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <span v-on:click="logOut"> ログアウト </span>
          </div>
        </div>
        <div
...
export default {
  ...
  async created() {
    this.getDataFromApi();
    this.$root.$on("refresh-profile-data", () => {
      this.getDataFromApi();
    });
  },
  methods: {
    getDataFromApi() {
      this.$store
        ...
        .then(() => {
          this.firstName = this.getStudent.first_name;
          this.familyName = this.getStudent.family_name;
        })
